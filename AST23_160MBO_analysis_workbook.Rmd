...
# min filter so that calculating MT won't crash
...
```{r}
# Filter with 500 and less than 20% mitochondria
AST23f <- subset(AST23, subset = nFeature_RNA > 500 & nFeatures < 10000 & percent.mt < 20)
AST23f
AST23isof <- subset(AST23iso, subset = nFeature_RNA > 500 & nFeatures < 10000 &percent.mt < 20)
AST23isof
MBO <- merge(AST23f, y=AST23isof, add.cell.ids = c("AST23","Isogenic"))
MBO

```

Align data from two samples
...
# create an object with the data ana anchors
MBO.anchors <- FindIntegrationAnchors(object.list = MBO.list, anchor.features = features)
# create a slot with the integration assay in a seurat object
...
# for expression analysis I will change the assay back to 'RNA'
...
# set k for SNN to be close to the square root of the number of starting cells 
MBO.combined <- FindNeighbors(MBO.combined, reduction = "pca", dims = 1:25, k.param = 63)
MBO.combined <- FindClusters(MBO.combined, resolution = c(0,0.1,0.3,0.6,0.9,1.2,1.8))
clustree(MBO.combined, prefix = "integrated_snn_res.") + theme(legend.position = "bottom")
# in fact the higher resolution just splits the neural population into subgroups - type of neurons 5 groups and neural stem cells NPC and neural plate split at lower resolutions.  
...
DimPlot(MBO.combined, reduction = "umap", repel = TRUE, label = TRUE, group.by = "integrated_snn_res.0.1")
DimPlot(MBO.combined, reduction = "umap", repel = TRUE, label = TRUE, group.by = "integrated_snn_res.0.9")
